...
void D3D12GSRender::PrepareRenderTargets(ID3D12GraphicsCommandList *copycmdlist)
{
	...
	u32 address_a = rsx::get_address(rsx::method_registers[mr_color_offset[0]], rsx::method_registers[mr_color_dma[0]]);
	u32 address_b = rsx::get_address(rsx::method_registers[mr_color_offset[1]], rsx::method_registers[mr_color_dma[1]]);
	u32 address_c = rsx::get_address(rsx::method_registers[mr_color_offset[2]], rsx::method_registers[mr_color_dma[2]]);
	u32 address_d = rsx::get_address(rsx::method_registers[mr_color_offset[3]], rsx::method_registers[mr_color_dma[3]]);
	u32 address_z = rsx::get_address(rsx::method_registers[NV4097_SET_SURFACE_ZETA_OFFSET], rsx::method_registers[NV4097_SET_CONTEXT_DMA_ZETA]);

	// Exit early if there is no rtt changes
	if ((m_surface.format == surface_format) &&
		(m_previous_address_a == address_a) &&
		(m_previous_address_b == address_b) &&
		(m_previous_address_c == address_c) &&
		(m_previous_address_d == address_d) &&
	...
	std::array<float, 4> clear_colors =
	{
		clear_r / 255.f,
		clear_g / 255.f,
		clear_b / 255.f,
	...
	{
		ID3D12Resource *rttA = m_rtts.bindAddressAsRenderTargets(m_device.Get(), copycmdlist, 0, address_a, m_surface.width, m_surface.height, m_surface.color_format,
			clear_colors);
	...
	{
		ID3D12Resource *rttB = m_rtts.bindAddressAsRenderTargets(m_device.Get(), copycmdlist, 0, address_b, m_surface.width, m_surface.height, m_surface.color_format,
			clear_colors);
	...
	case CELL_GCM_SURFACE_TARGET_MRT1:
	{
		...
		ID3D12Resource *rttB = m_rtts.bindAddressAsRenderTargets(m_device.Get(), copycmdlist, 1, address_b, m_surface.width, m_surface.height, m_surface.color_format,
			clear_colors);
		m_device->CreateRenderTargetView(rttB, &rttViewDesc, Handle);
	}
	break;
	case CELL_GCM_SURFACE_TARGET_MRT2:
	{
		...
		m_device->CreateRenderTargetView(rttB, &rttViewDesc, Handle);
		Handle.ptr += g_RTTIncrement;
		ID3D12Resource *rttC = m_rtts.bindAddressAsRenderTargets(m_device.Get(), copycmdlist, 2, address_c, m_surface.width, m_surface.height, m_surface.color_format,
			clear_colors);
	...
	case CELL_GCM_SURFACE_TARGET_MRT3:
	{
		...
		m_device->CreateRenderTargetView(rttC, &rttViewDesc, Handle);
		Handle.ptr += g_RTTIncrement;
		ID3D12Resource *rttD = m_rtts.bindAddressAsRenderTargets(m_device.Get(), copycmdlist, 3, address_d, m_surface.width, m_surface.height, m_surface.color_format,
			clear_colors);
	...
	ID3D12Resource *ds = m_rtts.bindAddressAsDepthStencil(m_device.Get(), copycmdlist, address_z, m_surface.width, m_surface.height, m_surface.depth_format, 1., 0);
