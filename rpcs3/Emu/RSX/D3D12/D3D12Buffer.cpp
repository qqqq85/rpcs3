...
void D3D12GSRender::upload_vertex_attributes(const std::vector<std::pair<u32, u32> > &vertex_ranges)
{
	...
	for (int index = 0; index < rsx::limits::vertex_count; ++index)
	{
		...
		assert(m_buffer_data.can_alloc(buffer_size));
		size_t heap_offset = m_buffer_data.alloc(buffer_size);

		void *buffer;
		ThrowIfFailed(m_buffer_data.m_heap->Map(0, &CD3DX12_RANGE(heap_offset, heap_offset + buffer_size), (void**)&buffer));
		...
		m_buffer_data.m_heap->Unmap(0, &CD3DX12_RANGE(heap_offset, heap_offset + buffer_size));

		D3D12_VERTEX_BUFFER_VIEW vertex_buffer_view =
		{
			m_buffer_data.m_heap->GetGPUVirtualAddress() + heap_offset,
			(UINT)buffer_size,
			(UINT)element_size
		};
...
{
	assert(m_buffer_data.can_alloc(256));
	size_t heap_offset = m_buffer_data.alloc(256);

	// Scale offset buffer
	// Separate constant buffer
	void *mapped_buffer;
	ThrowIfFailed(m_buffer_data.m_heap->Map(0, &CD3DX12_RANGE(heap_offset, heap_offset + 256), &mapped_buffer));
	...
	m_buffer_data.m_heap->Unmap(0, &CD3DX12_RANGE(heap_offset, heap_offset + 256));
...
void D3D12GSRender::upload_and_bind_vertex_shader_constants(size_t descriptor_index)
{
	...
	void *mapped_buffer;
	ThrowIfFailed(m_buffer_data.m_heap->Map(0, &CD3DX12_RANGE(heap_offset, heap_offset + buffer_size), &mapped_buffer));
